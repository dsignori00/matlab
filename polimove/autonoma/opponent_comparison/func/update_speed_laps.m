function update_speed_laps
    ...
    fig2 = findobj('Name','Acceleration Profile');
    axB = getappdata(fig2,'ax_a'); axC = getappdata(fig2,'ax_ax'); axA = getappdata(fig2,'ax_ay');
    ...
    linkaxes([ax_speed, ax_lapdiff, axB, axC, axA], 'off');
    ...
    [uI, ia] = unique(double(ego.s), 'stable');  
    ...
    for kk = selected_opps(:)'
        ...
        plot(ax_speed, opp_s, opp_speed, ...
             'Color', shared.colors(kk+1, :), ...
             'DisplayName', sprintf('%d - %s - Laptime: %s', kk, opp_name, laptime))

        if ~isempty(uI)
            egoInterp = interp1(uI, egoLT(ia), opp_s, 'linear', 'extrap');
            lap_time_diff = opp_lap_time - egoInterp;
            plot(ax_lapdiff, opp_s, lap_time_diff, ...
                 'Color', shared.colors(kk+1,:), ...
                 'DisplayName', sprintf('%d - %s - Laptime: %s', kk, opp_name, laptime));
        end
    end
    ...
    for k = 1:numel(shared.turn_s)
        xline(ax_speed, shared.turn_s(k), '--', 'Label', num2str(k), shared.line_opt{:});
        xline(ax_lapdiff, shared.turn_s(k), '--', 'Label', num2str(k), shared.line_opt{:});
    end
    ...
    linkaxes([ax_speed, ax_lapdiff, axB, axC, axA], 'x');
end