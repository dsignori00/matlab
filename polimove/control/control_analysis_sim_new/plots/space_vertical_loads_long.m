...
h.Name              = "[Log" + i_log + "] Space: vertical loads (F/R)";
tcl                 = tiledlayout(4,1);
...
ylabel('Front load [kg]');
for i = 1 : length(lap_idxs)
    data_interp = interp1(log.vehicle_fbk.bag_stamp, ...
                          log.vehicle_fbk.fl_load_wheel + log.vehicle_fbk.fr_load_wheel, ...
                          log.traj_server.bag_stamp(lap_idxs{i}));
    plot(log.traj_server.s(lap_idxs{i}), data_interp, 'Color', colors_lap{i}, 'DisplayName', ['lap ' num2str(log.traj_server.lap_count(lap_idxs{i}(1)))]);
end
...
for i = 1 : length(lap_idxs)
    data_interp = interp1(log.vehicle_fbk.bag_stamp, ...
                          log.vehicle_fbk.rl_load_wheel + log.vehicle_fbk.rr_load_wheel, ...
                          log.traj_server.bag_stamp(lap_idxs{i}));
    plot(log.traj_server.s(lap_idxs{i}), data_interp, 'Color', colors_lap{i}, 'DisplayName', ['lap ' num2str(log.traj_server.lap_count(lap_idxs{i}(1)))]);
end
...
for i = 1 : length(lap_idxs)
    data_interp = interp1(log.vehicle_fbk.bag_stamp, ...
                          100 * (log.vehicle_fbk.fl_load_wheel + log.vehicle_fbk.fr_load_wheel) ./ (log.vehicle_fbk.fl_load_wheel + log.vehicle_fbk.fr_load_wheel + log.vehicle_fbk.rl_load_wheel + log.vehicle_fbk.rr_load_wheel), ...
                          log.traj_server.bag_stamp(lap_idxs{i}));
    plot(log.traj_server.s(lap_idxs{i}), data_interp, 'Color', colors_lap{i}, 'DisplayName', ['lap ' num2str(log.traj_server.lap_count(lap_idxs{i}(1)))]);
end
