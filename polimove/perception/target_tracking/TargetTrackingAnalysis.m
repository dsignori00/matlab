...
lid_clust.sensor_stamp = log.perception__lidar__clustering_detections.sensor_stamp__tot;
...
rad_clust.sensor_stamp = log.perception__radar__clustering_detections.sensor_stamp__tot;
...
cam_yolo.sensor_stamp = log.perception__camera__yolo_detections.sensor_stamp__tot;
...
lid_pp.sensor_stamp = log.perception__lidar__pointpillars_detections.sensor_stamp__tot;
...
cam_yolo.stamp(cam_yolo.stamp < 0) = NaN;
...
% GROUND TRUTH
if(use_sim_ref)
    tt.stamp_ref = log.sim_out.bag_stamp;
    % relative
    tt.x_rel_ref = log.sim_out.opponents__x_rel(:,1);
    tt.y_rel_ref = log.sim_out.opponents__y_rel(:,1);
    tt.rho_dot_ref = log.sim_out.opponents__rho_dot(:,1);
    % map
    tt.x_map_ref = log.sim_out.opponents__x_geom(:,1);
    tt.y_map_ref = log.sim_out.opponents__y_geom(:,1);
    tt.vx_ref = log.sim_out.opponents__vx(:,1);
    tt.ax_ref = log.sim_out.opponents__ax(:,1);
    tt.yaw_map_ref = log.sim_out.opponents__psi(:,1);
    tt.yaw_map_ref = deg2rad(UnwrapPi(rad2deg(tt.yaw_map_ref)));
elseif(use_ref)
...
% f=f+1;
% hold on;
% area(tt.stamp,log.perception__opponents.opponents__rad_clust.meas(:,1:max_opp),'FaceColor',col.radar,'EdgeColor',col.radar,'DisplayName','Rad Clust');
% area(tt.stamp,log.perception__opponents.opponents__lid_pp.meas(:,1:max_opp),'FaceColor',col.pointpillars,'EdgeColor',col.pointpillars,'DisplayName','Lid PP');
% area(tt.stamp,log.perception__opponents.opponents__lid_clust.meas(:,1:max_opp),'FaceColor',col.lidar,'EdgeColor',col.lidar,'DisplayName','Lid Clust');
% area(tt.stamp,log.perception__opponents.opponents__cam_yolo.meas(:,1:max_opp),'FaceColor',col.camera,'EdgeColor',col.camera,'DisplayName','Camera');
% grid on;
% title('Count')
% legend
% 
...
plot(lid_clust.stamp,lid_clust.x_rel(:,1:max_opp),'o','MarkerFaceColor',col.lidar,'MarkerEdgeColor',col.lidar,'MarkerSize',sz,'DisplayName','Lid Clust');
plot(rad_clust.stamp,rad_clust.x_rel(:,1:max_opp),'o','MarkerFaceColor',col.radar,'MarkerEdgeColor',col.radar,'MarkerSize',sz,'DisplayName','Rad Clust');
plot(cam_yolo.stamp,cam_yolo.x_rel(:,1:max_opp),'o','MarkerFaceColor',col.camera,'MarkerEdgeColor',col.camera,'MarkerSize',sz,'DisplayName','Camera');
plot(lid_pp.stamp,lid_pp.x_rel(:,1:max_opp),'o','MarkerFaceColor',col.pointpillars,'MarkerEdgeColor',col.pointpillars,'MarkerSize',sz,'DisplayName','Lid PP');
plot(tt.stamp, tt.x_rel(:,1:max_opp), 'Color',col.tt,'DisplayName','tt');
if(use_ref || use_sim_ref); plot(tt.stamp_ref, tt.x_rel_ref, 'Color',col.ref,'DisplayName','Ground Truth'); end
grid on; title('x rel [m]'); legend

% pos y
axes(f) = nexttile([1,1]); f=f+1; hold on;
plot(lid_clust.stamp,lid_clust.y_rel(:,1:max_opp),'o','MarkerFaceColor',col.lidar,'MarkerEdgeColor',col.lidar,'MarkerSize',sz,'DisplayName','Lid Clust');
plot(rad_clust.stamp,rad_clust.y_rel(:,1:max_opp),'o','MarkerFaceColor',col.radar,'MarkerEdgeColor',col.radar,'MarkerSize',sz,'DisplayName','Rad Clust');
plot(cam_yolo.stamp,cam_yolo.y_rel(:,1:max_opp),'o','MarkerFaceColor',col.camera,'MarkerEdgeColor',col.camera,'MarkerSize',sz,'DisplayName','Camera');
plot(lid_pp.stamp,lid_pp.y_rel(:,1:max_opp),'o','MarkerFaceColor',col.pointpillars,'MarkerEdgeColor',col.pointpillars,'MarkerSize',sz,'DisplayName','Lid PP');
plot(tt.stamp, tt.y_rel(:,1:max_opp), 'Color',col.tt,'DisplayName','tt');
if(use_ref || use_sim_ref); plot(tt.stamp_ref, tt.y_rel_ref, 'Color',col.ref,'DisplayName','Ground Truth'); end
grid on; title('y rel [m]'); legend

% rho dot
axes(f) = nexttile([1,2]); f=f+1; hold on;
plot(rad_clust.stamp,rad_clust.rho_dot(:,1:max_opp),'o','MarkerFaceColor',col.radar,'MarkerEdgeColor',col.radar,'MarkerSize',sz,'DisplayName','Rad Clust');
plot(tt.stamp, tt.rho_dot(:,1:max_opp), 'Color',col.tt,'DisplayName','tt');
if(use_ref || use_sim_ref); plot(tt.stamp_ref, tt.rho_dot_ref, 'Color',col.ref,'DisplayName','Ground Truth'); end
grid on; title('rho dot [m/s]'); legend
...
% pos X
axes(f) = nexttile([1,1]); f=f+1; hold on;
plot(lid_clust.stamp,lid_clust.x_map(:,1:max_opp),'o','MarkerFaceColor',col.lidar,'MarkerEdgeColor',col.lidar,'MarkerSize',sz,'DisplayName','Lid Clust');
plot(rad_clust.stamp,rad_clust.x_map(:,1:max_opp),'o','MarkerFaceColor',col.radar,'MarkerEdgeColor',col.radar,'MarkerSize',sz,'DisplayName','Rad Clust');
plot(cam_yolo.stamp,cam_yolo.x_map(:,1:max_opp),'o','MarkerFaceColor',col.camera,'MarkerEdgeColor',col.camera,'MarkerSize',sz,'DisplayName','Camera');
plot(lid_pp.stamp,lid_pp.x_map(:,1:max_opp),'o','MarkerFaceColor',col.pointpillars,'MarkerEdgeColor',col.pointpillars,'MarkerSize',sz,'DisplayName','Lid PP');
plot(tt.stamp,tt.x_map(:,1:max_opp),'Color',col.tt,'DisplayName','tt');
if(use_ref || use_sim_ref); plot(tt.stamp_ref,tt.x_map_ref,'Color',col.ref,'DisplayName','Ground Truth'); end
grid on; title('x map [m]'); legend

% pos y
axes(f) = nexttile([1,1]); f=f+1; hold on;
plot(lid_clust.stamp,lid_clust.y_map(:,1:max_opp),'o','MarkerFaceColor',col.lidar,'MarkerEdgeColor',col.lidar,'MarkerSize',sz,'DisplayName','Lid Clust');
plot(rad_clust.stamp,rad_clust.y_map(:,1:max_opp),'o','MarkerFaceColor',col.radar,'MarkerEdgeColor',col.radar,'MarkerSize',sz,'DisplayName','Rad Clust');
plot(cam_yolo.stamp,cam_yolo.y_map(:,1:max_opp),'o','MarkerFaceColor',col.camera,'MarkerEdgeColor',col.camera,'MarkerSize',sz,'DisplayName','Camera');
plot(lid_pp.stamp,lid_pp.y_map(:,1:max_opp),'o','MarkerFaceColor',col.pointpillars,'MarkerEdgeColor',col.pointpillars,'MarkerSize',sz,'DisplayName','Lid PP');
plot(tt.stamp,tt.y_map(:,1:max_opp),'Color',col.tt,'DisplayName','tt');
if(use_ref || use_sim_ref); plot(tt.stamp_ref,tt.y_map_ref,'Color',col.ref,'DisplayName','Ground Truth'); end
grid on; title('y map [m]'); legend

% vx
axes(f) = nexttile([1,1]); f=f+1; hold on;
plot(tt.stamp,tt.vx(:,1:max_opp),'Color',col.tt,'DisplayName','tt');
if(use_ref || use_sim_ref); plot(tt.stamp_ref,tt.vx_ref,'Color',col.ref,'DisplayName','Ground Truth'); end
grid on; title('vx [m/s]'); legend

% ax
axes(f) = nexttile([1,1]); f=f+1; hold on;
plot(tt.stamp,tt.ax(:,1:max_opp),'Color',col.tt,'DisplayName','tt');
if(use_sim_ref); plot(tt.stamp_ref,tt.ax_ref,'Color',col.ref,'DisplayName','Ground Truth'); end
grid on; title('ax [m/s^2]'); legend
...
axes(f) = nexttile([1,2]); f=f+1; hold on;
plot(lid_clust.stamp,lid_clust.yaw_map(:,1:max_opp),'o','MarkerFaceColor',col.lidar,'MarkerEdgeColor',col.lidar,'MarkerSize',sz,'DisplayName','Lid Clust');
plot(rad_clust.stamp,rad_clust.yaw_map(:,1:max_opp),'o','MarkerFaceColor',col.radar,'MarkerEdgeColor',col.radar,'MarkerSize',sz,'DisplayName','Rad Clust');
plot(cam_yolo.stamp,cam_yolo.yaw_map(:,1:max_opp),'o','MarkerFaceColor',col.camera,'MarkerEdgeColor',col.camera,'MarkerSize',sz,'DisplayName','Camera');
plot(lid_pp.stamp,lid_pp.yaw_map(:,1:max_opp),'o','MarkerFaceColor',col.pointpillars,'MarkerEdgeColor',col.pointpillars,'MarkerSize',sz,'DisplayName','Lidar PP');
plot(tt.stamp,tt.yaw_map(:,1:max_opp),'Color',col.tt,'DisplayName','tt');
if(use_ref || use_sim_ref)
    tt.yaw_map_ref = mod(tt.yaw_map_ref,2*pi);
    plot(tt.stamp_ref,tt.yaw_map_ref,'Color',col.ref,'DisplayName','Ground Truth');
end
grid on; title('yaw [deg]'); legend
...
function refreshTimeButtonPushed(src,event)
    ...
    lid_clust.stamp = evalin('base', 'lid_clust.stamp');
    lid_clust.x_map = evalin('base', 'lid_clust.x_map');
    lid_clust.y_map = evalin('base', 'lid_clust.y_map');
    rad_clust.stamp = evalin('base', 'rad_clust.stamp');
    rad_clust.x_map = evalin('base', 'rad_clust.x_map');
    rad_clust.y_map = evalin('base', 'rad_clust.y_map');
    cam_yolo.stamp = evalin('base', 'cam_yolo.stamp');
    cam_yolo.x_map = evalin('base', 'cam_yolo.x_map');
    cam_yolo.y_map = evalin('base', 'cam_yolo.y_map');
    lid_pp.stamp = evalin('base', 'lid_pp.stamp');
    lid_pp.x_map = evalin('base', 'lid_pp.x_map');
    lid_pp.y_map = evalin('base', 'lid_pp.y_map');
    tt.stamp=evalin('base','tt.stamp');
    tt.x_map = evalin('base', 'tt.x_map');
    tt.y_map = evalin('base', 'tt.y_map');
    if(use_ref || use_sim_ref)
        tt.stamp_ref=evalin('base','tt.stamp_ref');
        tt.x_map_ref = evalin('base', 'tt.x_map_ref');
        tt.y_map_ref = evalin('base', 'tt.y_map_ref');
    end


    t_lim=xlim(axes(1));
    t1_lid_clust = find(lid_clust.stamp>t_lim(1),1);
    tend_lid_clust = find(lid_clust.stamp<t_lim(2),1,'last');
    t1_rad_clust = find(rad_clust.stamp>t_lim(1),1);
    tend_rad_clust = find(rad_clust.stamp<t_lim(2),1,'last');
    t1_cam_yolo = find(cam_yolo.stamp>t_lim(1),1);
    tend_cam_yolo = find(cam_yolo.stamp<t_lim(2),1,'last');
    t1_lid_pp = find(lid_pp.stamp>t_lim(1),1);
    tend_lid_pp = find(lid_pp.stamp<t_lim(2),1,'last');
