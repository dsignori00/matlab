...
for i=1:length(labels)
    tmp1 = eval(strcat('log.estimation.',string(labels_full(i))));
    labels_value(i,:) = tmp1(1,1);
end
...
function refreshTimeButtonPushed(src,event)
    ...
    ekf_x = evalin('caller', 'log.estimation.ekf_x_cog');
    ekf_y = evalin('caller', 'log.estimation.ekf_y_cog');
    ol_x = evalin('caller', 'log.estimation.output_filter_x_cog');
    ol_y = evalin('caller', 'log.estimation.output_filter_y_cog');
    ...
    plot(ekf_x(t1_est:tend_est), ekf_y(t1_est:tend_est),'Color',colref,'displayname','ekf');
    plot(ol_x(t1_est:tend_est), ol_y(t1_est:tend_est),'Color',col2,'displayname','open loop');
