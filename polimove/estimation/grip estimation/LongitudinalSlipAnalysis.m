...
r_uniq = -log.vectornav__raw__common__imu_rate__z(idx);
...
ay_uniq = -log.vectornav__raw__common__imu_accel__y(idx);
...
r_filt = lsim(filt_tf,r_interp,t);
rdot_filt = lsim(filt_tf_rdot,r_interp,t);
angle_filt = lsim(filt_tf,angle_interp,t);
ax_filt = lsim(filt_tf,ax_interp,t);
ay_filt = lsim(filt_tf,ay_interp,t);

temp_rl_filt = lsim(filt_tf,temp_rl_interp,t);
brake_rl_filt = lsim(filt_tf,brake_rl_interp,t);
brake_temp_rl_filt = lsim(filt_tf,brake_temp_rl_interp,t);

load_f_meas_filt = lsim(filt_tf,load_f_meas,t);
load_r_meas_filt = lsim(filt_tf,load_r_meas,t);

gearshift_filt = lsim(filt_tf,gearshift,t);


stroke_r3rd_filt = lsim(filt_tf,stroke_r3rd_interp,t);
stroke_rl_filt = lsim(filt_tf,stroke_rl_interp,t);
ride_filt = lsim(filt_tf,ride_interp,t);

ome_fl_filt = lsim(filt_tf,ome_fl_interp,t);
ome_fr_filt = lsim(filt_tf,ome_fr_interp,t);
ome_rl_filt = lsim(filt_tf,ome_rl_interp,t);
ome_rr_filt = lsim(filt_tf,ome_rr_interp,t);
...
ome_dot_fl_filt = lsim(filt_tf_rdot,ome_fl_interp,t);
ome_dot_rl_filt = lsim(filt_tf_rdot,ome_rl_interp,t);
ome_dot_rr_filt = lsim(filt_tf_rdot,ome_rr_interp,t);
...
Fx_f = ax_filt*M*(bb)/(bb*Rf+Rr-bb*Rr)*Rf+F_drag*(TOT_LENGTH-FRONT_LENGTH)/TOT_LENGTH;
...
Fx_fl = min(0,Fx_f) + 2*F_inertia_fl;
Fx_rl = Fx_r + max(0,Fx_r) + F_inertia_rl;
Fx_rr = Fx_r + max(0,Fx_r) + F_inertia_rr;
...
lambda_fr(vx_filt<0.1)=0;
lambda_fl(vx_filt<0.1)=0;
lambda_rr(vx_filt<0.1)=0;
lambda_rl(vx_filt<0.1)=0;

%% analysis
valid_idx=and(abs(ay_filt)<2, vx_filt>5);
...
figure;
hold on
scatter(lambda_rl(valid_idx),Fx_rl(valid_idx)./load_r(valid_idx), 10, s_interp(valid_idx),"filled");
% scatter(lambda_rr(valid_idx),Fx_rr(valid_idx)./load_r(valid_idx), 10, s_interp(valid_idx),"filled");
grid on;
...
figure;
hold on
scatter(lambda_fl(valid_idx),Fx_fl(valid_idx)./load_f(valid_idx), 10, s_interp(valid_idx),"filled");
% scatter(lambda_fr(valid_idx),Fx_f(valid_idx)./load_f(valid_idx), 10, s_interp(valid_idx),"filled");
grid on;
