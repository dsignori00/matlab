...
% kistler
t_kist = double(log.kistler_msg__correvit_stamp__sec)+double(log.kistler_msg__correvit_stamp__nanosec)*1e-9 -double(log.time_offset_nsec)*1e-9+0.01;
...
gearshift = interp1(t_vfbk, double(log.vehicle_fbk_eva__requested_gear_ack), t, 'linear');
...
F_drag = 0.5*Cd*rho_air.*vx_filt.^2 *0.8;

Izz=0;
F_inertia_fl = ome_dot_fl_filt/Rf*Izz;
F_inertia_fr = ome_dot_fr_filt/Rf*Izz;
F_inertia_rl = ome_dot_rl_filt/Rr*Izz;
F_inertia_rr = ome_dot_rr_filt/Rr*Izz;
% 
% F_inertia_rl = 0;
% F_inertia_rr = 0;
...
change_idx = [false; diff(gearshift)' ~= 0];

shifting_mask = true(size(gearshift));

for i = find(change_idx)'
    if i<N0
        continue
    end
    shifting_mask(i-N0:min(i-N0+N, end)) = false;
end
%% analysis
...
valid_idx=and(valid_idx,shifting_mask);
% valid_idx=and(valid_idx,ax_filt<0);
...
nexttile;
hold on
scatter(lambda_rl(valid_idx),Fx_rl(valid_idx)./(load_r(valid_idx)/2), 10, t(valid_idx),"filled");
% scatter(lambda_rr(valid_idx),Fx_rr(valid_idx)./(load_r(valid_idx)/2), 10, t(valid_idx),"filled");
