...
% Anello
an_t = log.APIMU.bag_stamp;
an_ax = -log.APIMU.ay*9.81;
an_ay = -log.APIMU.ax*9.81;
an_az = (-log.APIMU.az)*9.81;
an_tot = sqrt(an_ax.^2 + an_ay.^2 + an_az.^2);
...
plot(an_t,an_ax,'DisplayName','an');
legend show
...
plot(an_t,an_ay,'DisplayName','an');
legend show
xlabel('Time [s]')
...
plot(an_t,an_az,'DisplayName','an');
legend show
xlabel('Time [s]')
...
[an_ps_ax, an_ps_ax_fx] = pspectrum(an_ax,an_t);
[an_ps_ay, an_ps_ax_fy] = pspectrum(an_ay,an_t);
[an_ps_az, an_ps_ax_fz] = pspectrum(an_az,an_t);
...
plot(an_ps_ax_fx, 10*log10(an_ps_ax),'DisplayName','an');
% plot(an_ps_ax_fx, flip(10*log10(an_ps_ax)),'DisplayName','an');
legend show
title('ax')
set(gca, 'XScale', 'log');
...
plot(an_ps_ax_fy, 10*log10(an_ps_ay),'DisplayName','an');
legend show
title('ay')
set(gca, 'XScale', 'log');
...
plot(an_ps_ax_fz, 10*log10(an_ps_az),'DisplayName','an');
legend show
title('az')
set(gca, 'XScale', 'log');
...
[an_ps_tot, an_ps_ax_ft] = pspectrum(an_tot,an_t);
...
plot(an_t,an_tot,'DisplayName','an');
legend show
title('time')
xlabel('time [s]');
...
plot(an_ps_ax_ft, 10*log10(an_ps_tot),'DisplayName','an');
legend show
...
plot(an_t(1:end-1), (an_t(2:end)-an_t(1:end-1)),'DisplayName','an');
legend show
...
histogram(an_t(2:end)-an_t(1:end-1),N,'Normalization','probability','DisplayName','an')
legend show
...
% plot(an_t(1:end-1), cumsum(an_tot(1:end-1).*(an_t(2:end)-an_t(1:end-1))),'DisplayName','an');
% legend show
...
histogram(an_tot,N,'Normalization','probability','DisplayName','an')
