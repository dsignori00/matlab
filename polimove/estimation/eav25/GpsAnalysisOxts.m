...
% OXTS 1
o1_t = log.ins__nav_sat_fix.bag_stamp;
o1_x = log.ins__nav_sat_fix.latitude;
o1_y = log.ins__nav_sat_fix.longitude;
o1_z = log.ins__nav_sat_fix.altitude;
[o1_x, o1_y, ~] = geodetic2enu(o1_x, o1_y, o1_z,lat0,lon0,h0,wgs84);
o1_fix = zeros(length(log.ins__nav_sat_fix.status__status),1);
o1_fix(log.ins__nav_sat_fix.status__status==2)=1;
...
plot(o1_t,o1_x,'DisplayName','anello');
legend show
...
plot(o1_t,o1_y,'DisplayName','anello');
legend show
ylabel('Long [m]');
xlabel('Time [s]')
...
plot(o1_t,o1_fix,'DisplayName','anello');
legend show
ylabel('RTK fix');
xlabel('Time [s]');
ylim([-0.2, 1.2]);
...
hold on;
grid on;
vn1_fix_interp = interp1(double(vn1_t), double(vn1_fix), double(o1_t), 'previous', 'extrap');
greater_status1 = o1_fix > vn1_fix_interp;
a1_greater = ones(length(o1_fix),1);
a1_greater(~greater_status1) = 0;
area(o1_t, a1_greater, 'DisplayName', 'anello best','EdgeColor','none');
equal1_status = o1_fix == vn1_fix_interp;
equal1 = ones(length(o1_fix),1);
equal1(~equal1_status) = 0;
area(o1_t, equal1, 'DisplayName', 'equal','EdgeColor','none');
smaller_status1 = o1_fix < vn1_fix_interp;
a1_smaller = ones(length(o1_fix),1);
a1_smaller(~smaller_status1) = 0;
area(o1_t, a1_smaller, 'DisplayName', 'vectornav best','EdgeColor','none');
ylim([-0.2, 1.2]);
legend show
xlabel('Time [s]')
