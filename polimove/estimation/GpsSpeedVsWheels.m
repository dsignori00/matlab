...
plot(log.estimation__stamp__tot,log.estimation__vx_vehicle_speed_report__v(:,1)*0.99,'DisplayName','wheels_corr');
...
hold on;
scatter(v(fix),v(fix)./v_fl(fix),15,temp_fl(fix),'filled');
scatter(v(fix),v(fix)./v_fr(fix),15,temp_fr(fix),'filled');
ylim([0.7,1.3]);
title('Front');
legend show;
grid on;
