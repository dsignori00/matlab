function line_eq = convertLines(line_eq,out_line_form, LINEFORM_type)
    ...
    for i = 1:size(line_eq.coeff,1)
        for j = 1:size(line_eq.coeff,2)
            [line_eq.p1(i,j), line_eq.p2(i,j), line_eq.form(i,j)] = ...
                convertLineForm(line_eq.coeff(i,j,:), line_eq.form(i,j), out_line_form, LINEFORM_type, line_eq.p1(i,j), line_eq.p2(i,j) );
        end
    end
